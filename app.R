...
ui <- fluidPage(
  ...
  fluidRow(
    column(4,
      "Hi Mom",
      radioButtons("patterntype","Please Select Type of Pattern", c("Pattern Only","Timing Only",
                  "Pattern and Timing"),("Pattern Only")),
      actionButton("patplotgo","Generate Visual")
    ),
    column(4,
      "Input Step Information",
      conditionalPanel(
        condition = "input.patterntype == 'Pattern Only'",
        textInput('po_ap', "Arrow Pattern  (e.g. L U R L D R)", "1,3,4,1,2,4"),
      ...
      conditionalPanel(
        condition = "input.patterntype == 'Pattern and Timing'",
        textInput('pt_ap', "Arrow Pattern (e.g. L U R L D R)", "1,3,4,1,4,3,1"),
  ...
  fluidRow(
    ...
    column(4,
      actionButton("chart_viz","Generate Chart Visual, Will Currently throw an error if count for chart is 0")
    )
  ),
...
server <- function(input, output, session) {
  ...
  PL = eventReactive(input$patplotgo, {
    if(input$patterntype == "Pattern Only"){
    ...
    if(input$patterntype == "Pattern and Timing"){
  ...
  output$search_tab = renderDT({
    datatable(search_tab_gen(search_res(),a20p))
  })
  
  observeEvent(input$search_start, {
    updateSelectizeInput(session, inputId = "chart_to_viz", choices = filterlist())
  })
  
  observeEvent(input$chart_viz, {
    output$chartplot = renderPlot({
      patplot_shiny(input$chart_to_viz,search_res(),filterlist(), allcharts = allcharts, readbpm = 600, smod = NULL)
